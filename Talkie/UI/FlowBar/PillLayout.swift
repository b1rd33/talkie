...
/// width/2. The panel size is therefore computed here from bounded settings, never read
/// back from AppKit.
enum PillLayout {
    /// Base size for the standard pill and the default 44-point orb.
    static let panelSize = CGSize(width: 260, height: 56)
    static let margin: CGFloat = 12

    static func clampedOrbSize(_ value: Double) -> Double {
        value.isFinite ? min(96, max(28, value)) : 44
    }

    static func panelSize(style: PillStyle, orbSize: Double) -> CGSize {
        guard style == .thinkingOrb else { return panelSize }
        return CGSize(width: panelSize.width,
                      height: max(panelSize.height, ceil(clampedOrbSize(orbSize) * 1.18 + 4)))
    }
