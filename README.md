...
Talkie is a native macOS menu-bar app that turns speech into text at the cursor with hold-to-talk, hands-free, batch-cloud, and instant-streaming workflows.
...
The project does not currently publish or validate an Intel release. Treat Intel as unsupported until it has been built and exercised on Intel hardware.
...
Launch the built app from Xcode or the build products directory, then follow onboarding for Microphone and Accessibility access. Transcription requires an OpenAI or OpenRouter API key stored in the macOS Keychain.
...
Local transcription and automatic offline fallback have been removed. Saved local/offline profiles block recording and uploads until you explicitly select a cloud profile. Existing history, recordings, custom profiles and downloaded model files are preserved.
...
enabled. OpenRouter remains available for batch transcription.

### Shape the result

- **Profiles** apply a coherent engine, provider, model, and cleanup pipeline. Built-in profiles include Live Typing, Instant, Best Accuracy, and Cheapest Cloud; custom profiles can preserve tuned settings.
...
- **Dictionary** entries improve provider prompts and cleanup.
...
- **Language and microphone selection** let you pin an output language and choose an input device. Provider language coverage varies.
