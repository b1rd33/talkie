...
expect_pattern scripts/release.sh 'fail "set DEVELOPMENT_TEAM_ID' "release must require DEVELOPMENT_TEAM_ID"
expect_pattern scripts/release.sh 'fail "set SIGNING_IDENTITY' "release must require SIGNING_IDENTITY"
...
expect_pattern scripts/release.sh 'notarytool history' "environment validation must authenticate notarization credentials"
expect_pattern scripts/release.sh 'git diff --quiet' "release must reject dirty tracked provenance"
...
expect_pattern scripts/release.sh 'Authority=\$SIGNING_IDENTITY' "release must verify the exact signing authority"
expect_pattern scripts/release.sh 'TeamIdentifier=\$DEVELOPMENT_TEAM_ID' "release must verify the signature team"
expect_pattern scripts/release.sh 'CodeDirectory.*runtime' "release must verify the hardened-runtime signature flag"
expect_pattern scripts/release.sh '\^Timestamp=\.\+\$' "release must require a secure signature timestamp"
expect_pattern scripts/release.sh 'codesign -d --entitlements' "release must inspect signed production entitlements"
expect_pattern scripts/release.sh 'actual_entitlements_json.*expected_entitlements_json' "release must require exact production entitlements"
expect_pattern scripts/release.sh 'ditto -x -k' "release must extract and verify the final ZIP"
expect_pattern scripts/release.sh 'hdiutil attach' "release must mount and verify the final DMG"
expect_pattern scripts/release.sh '^[[:space:]]+-readonly' "release must mount the DMG read-only"
expect_pattern scripts/release.sh 'SHA256SUMS' "release must publish checksums"
expect_pattern scripts/release.sh 'release-metadata\.txt' "release must publish provenance metadata"
expect_pattern scripts/release.sh 'mktemp -d "build/\.release-\$version' "release must use private staging"
expect_pattern scripts/release.sh 'mv "\$output_dir" "\$final_dir"' "release must promote the completed artifact set atomically"
...
expect_pattern scripts/build-release-adhoc.sh 'ditto -x -k' "ad-hoc community preview must verify its extracted ZIP"
expect_pattern scripts/build-release-adhoc.sh 'cmp -s.*legal_notice' "ad-hoc community preview must verify exact legal notices"
